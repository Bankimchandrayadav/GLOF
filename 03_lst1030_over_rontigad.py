# %% [markdown]
# # About
# This code reports the LST time series over the Rontigad area
...
# [1] Read
ds1 = xr.open_dataset("../../004_RishiGanga/02_RS_data/01_Raster/01_LST/03_Rontigad/MOD11A1.006_1km_aid0001 (1).nc")
ds2 = xr.open_dataset("../../004_RishiGanga/02_RS_data/01_Raster/01_LST/03_Rontigad/MOD11A1.006_1km_aid0001.nc")
...
df = pd.DataFrame(index=ds.time.values, columns=['Rontigad'])

# [2] Get the LST time series into the dataframe
for i in td(range(len(df)), desc='Getting values'):
    df.Rontigad[i] = np.round((np.float32(ds.LST_Day_1km.isel(time=i).mean().values)),2)
...
# %% [markdown]
## Save to csv
df.to_csv("../../004_RishiGanga/05_CSVs/03_lst_over_rontigad.csv")
